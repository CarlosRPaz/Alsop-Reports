...
const TEAMS = ["Managers", "Sales", "Service", "EA"];
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6 shadow-sm">
      <div className="flex items-center justify-between mb-3">
        ...
        <div>
          {activeCount > 0 && (
            <button onClick={clearAll} className="text-[11px] text-slate-500 hover:text-slate-900 transition-colors">
              Clear All
            </button>
          )}
        </div>
      ...
      <div className="pt-3 border-t border-slate-100 flex flex-wrap gap-x-8 gap-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {OFFICES.map(office => {
              ...
              return (
                <button
                  key={office} 
                  onClick={() => toggleFilter("offices", office)}
            ...
            <div className="flex flex-wrap gap-1.5">
              {availableMeetings.map(mt => {
                ...
                return (
                  <button
                    key={mt} 
                    onClick={() => toggleFilter("meetings", mt)}
                    className={`px-2.5 py-1 rounded-md text-[11px] font-medium transition-colors border ${
                      active 
